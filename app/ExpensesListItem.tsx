...
const ExpensesListItem = ({ expense }: iProps) => {
  ...
  const deleteExpense = async (id: number) => {
    try {
      mutate(
        "/api/expenses",
        (data) => data.filter((expense: any) => expense.id !== id),
        false
      );
      const body = id;
  ...
  return (
    <li className={s.list_item}>
      <div>
        <button className={s.toggle_btn} onClick={toggleExtraInfo}>
          <GrFormDown />
        </button>
        {category.name}
      </div>
      <div className={s.amount}>
        ...
        <MdOutlineEuro
          color="gray"
          size={13}
          className={`${showExtraInfo ? s.rotate_up : null}`}
        />
      </div>
      <div className={`${s.extra_info} ${showExtraInfo && s.open}`}>
        {desc ? <div className={s.desc}>{desc}</div> : null}
        <p className={s.date}>
          <span suppressHydrationWarning>{formateDate(createdAt)}</span>
          <span suppressHydrationWarning>{formateHours(createdAt)}</span>
        </p>
        <ImBin
          size={15}
